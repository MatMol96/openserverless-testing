...
case "$TYPE" in
kind)
    if NUV_LOGIN=$user NUV_PASSWORD=$password ops -login http://localhost:3233 | grep "Successfully logged in as $user."; then
        echo SUCCESS LOGIN
    else
        echo FAIL LOGIN
        exit 1
...
*)
    ...
    if NUV_LOGIN=$user NUV_PASSWORD=$password ops -login $APIURL | grep "Successfully logged in as $user."; then
        echo SUCCESS LOGIN
    else
        echo FAIL LOGIN
        exit 1
