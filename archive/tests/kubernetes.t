  $ export EXTRA=D=1
Context:
  $ ops setup kubernetes context
  Using current context
  RUN: kubectl config get-contexts
  $ ops setup kubernetes context CONTEXT=
  Using current context
  RUN: kubectl config get-contexts
  $ ops setup kubernetes context CONTEXT=test
  RUN: kubectl config use-context test
  RUN: kubectl config get-contexts
Info
  $ ops setup kubernetes info | grep RUN:
  RUN: kubectl config get-contexts
  RUN: kubectl -n nuvolaris get no
  RUN: kubectl version --short
  $ ops setup kubernetes status | grep RUN:
  RUN: kubectl -n nuvolaris get sts,po,svc
Create
  $ ops setup kubernetes create | grep RUN:
  RUN: kubectl config get-contexts
  ...
  $ ops setup kubernetes create CONTEXT= | grep "kubectl config"
  RUN: kubectl config get-contexts
  $ ops setup kubernetes create CONTEXT=demo | grep "kubectl config"
